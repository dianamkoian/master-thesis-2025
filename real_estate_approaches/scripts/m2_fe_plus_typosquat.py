"""
M2-FE+ TYPOSQUAT — вариант M2-FE+ с добавленной Карининой группой typosquat (Deng et al., 2020).

Это копия m2_fe_plus.py + добавление 3 признаков из домена К.У. Азимовой:
  - brand_exact   = (brand ∈ name) ? 1 : 0
  - brand_fuzzy   = partial_ratio(brand, name) / 100
  - typosquat     = max(0, brand_fuzzy − 0.5 * brand_exact)

Мотивация: orthogonality analysis показала Pearson(typosquat, RMM_proba) ≈ −0.07,
то есть сигнал ОРТОГОНАЛЕН существующему RMM-каналу. Цель — feature-level
интеграция Карининого вклада в RMM (channel-уровень остаётся M2-FE+ Дианы).

Output:
  notebooks/test_proba_diana_m2_fe_plus_typosquat.npy
  notebooks/test_proba_diana_m2_fe_plus_typosquat_calibrated.npy
  artifacts/cdsm_v3/rmm_catboost_typosquat.cbm
"""
...
# ---------- Группа №5 Typosquat (Deng et al., 2020 / К.У. Азимова) ----------
log('Группа №5: typosquat / brand identification deception (Karina Group D)')
from rapidfuzz.fuzz import partial_ratio
brand_str_arr = df['brand_str'].str.lower().values
name_str_arr = df['name_rus_str'].str.lower().values
df['brand_exact'] = np.array(
    [(b in n) and len(b) > 0 for b, n in zip(brand_str_arr, name_str_arr)],
    dtype=np.float32,
)
df['brand_fuzzy'] = np.array(
    [partial_ratio(b, n) / 100.0 if (b and n) else 0.0 for b, n in zip(brand_str_arr, name_str_arr)],
    dtype=np.float32,
)
df['typosquat'] = np.maximum(0, df['brand_fuzzy'].values - 0.5 * df['brand_exact'].values).astype(np.float32)
log('  typosquat готово (3 признака)')
...
new_feat_groups = {
    ...
    'typosquat': ['brand_exact', 'brand_fuzzy', 'typosquat'],
}
...
# Save — TYPOSQUAT VARIANT (отдельные файлы, оригинал не перезаписывается)
np.save(OUT_DIR / 'test_proba_diana_m2_fe_plus_typosquat.npy', p_test_raw)
np.save(OUT_DIR / 'test_proba_diana_m2_fe_plus_typosquat_calibrated.npy', p_test_cal)
