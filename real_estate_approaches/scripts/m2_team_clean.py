"""
Team split — ОЧИЩЕННАЯ версия (m2_team_clean.py).

Удалены 4 признака для полной чистоты headline-конфигурации:
  - brand_exact / brand_fuzzy / typosquat (концепция typosquat относится
    к домену мобильных приложений, К. Азимова);
  - clip_cat_sim (cosine с category centroid; для перестраховки убран,
    хотя формально это аналог K-means structural в CLIP-пространстве).

Итого: 53 tab + 512 CLIP + 100 SVD = 665 признаков.
Сохраняет:
  - test_proba_diana_team_clean.npy
  - val_proba_diana_team_clean.npy
Оригинальные artefactы test_proba_real_estate.npy / test_proba_diana_team.npy
НЕ перезаписываются (нужны Соне для общего стэкинга — см. § 5.4-5.6).
"""
import gc
import time
import warnings
...
np.save(OUT_NB / 'test_proba_diana_team_clean_calibrated.npy', p_test.astype(np.float32))
