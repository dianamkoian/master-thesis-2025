"""
Team split v2 — версия скриптом для экономии памяти.
Делает: K-means structural + typosquatting + clip_cat_sim + isotonic calibration.
БЕЗ e5-small (он отдельно если нужно), вместо него — TF-IDF SVD с 100 компонентами.
"""
import gc
import time
import warnings
...
# Typosquat
t0 = time.time()
print('  Computing typosquat (~1-2 min)...')
...
print(f'  typosquat done in {time.time()-t0:.1f}s')
...
tabular_features = [
    ...
    'brand_exact', 'brand_fuzzy', 'typosquat', 'clip_cat_sim',
...
# Save — RAW (without isotonic calibration) for stacking
# Sonya's meta-classifier handles calibration itself; raw probas have richer tail distribution
out_proba = OUT_DIR / 'test_proba_real_estate.npy'
np.save(out_proba, p_test_raw.astype(np.float32))  # RAW для стекинга
np.save(OUT_DIR / 'val_proba_real_estate.npy', p_val_raw.astype(np.float32))
np.save(OUT_DIR / 'test_proba_real_estate_calibrated.npy', p_test.astype(np.float32))  # calibrated отдельно
