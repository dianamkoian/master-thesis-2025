"""
Team split — конфигурация M2 без LOO target encoding (clean, m2_no_te_clean.py).

Отличие от m2_team_clean.py: дополнительно удалены 2 LOO target encoding
признака `category_target_mean` и `brand_target_mean` для ablation TE
(§ 4.4.7 ВКР).

Итого: 51 tab + 512 CLIP + 100 SVD = 663 признака.
Сохраняет в notebooks/:
  - test_proba_no_te_clean.npy
  - test_proba_no_te_clean_calibrated.npy
  - val_proba_no_te_clean.npy
"""
import gc
import time
import warnings
...
def engineer(frame, refs):
    ...
    # LOO target encoding УДАЛЁН для ablation TE (§ 4.4.7)
...
out_proba = OUT_NB / 'test_proba_no_te_clean.npy'
np.save(out_proba, p_test_raw.astype(np.float32))
np.save(OUT_NB / 'val_proba_no_te_clean.npy', p_val_raw.astype(np.float32))
np.save(OUT_NB / 'test_proba_no_te_clean_calibrated.npy', p_test.astype(np.float32))
