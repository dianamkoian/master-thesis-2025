"""
M2 Ablation вклада каждой компоненты: переобучение M2 на ИНДИВИДУАЛЬНОМ split автора
...
LOG = ROOT / 'real_estate_approaches' / 'scripts' / 'm2_ablation_indiv_log.txt'
...
for cfg_name, feats in configs.items():
    ...
    np.save(OUT_DIR / f'test_proba_diana_ablation_{safe_name}_indiv.npy', p_te)
